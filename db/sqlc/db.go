...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	q := Queries{db: db}
	var err error
	if q.addUpdateAccountBalanceStmt, err = db.PrepareContext(ctx, addUpdateAccountBalance); err != nil {
		return nil, fmt.Errorf("error preparing query AddUpdateAccountBalance: %w", err)
	}
	...
	if q.listsEntriesStmt, err = db.PrepareContext(ctx, listsEntries); err != nil {
		return nil, fmt.Errorf("error preparing query ListsEntries: %w", err)
	}
	if q.listsTransfersStmt, err = db.PrepareContext(ctx, listsTransfers); err != nil {
		return nil, fmt.Errorf("error preparing query ListsTransfers: %w", err)
	}
...
func (q *Queries) Close() error {
	var err error
	if q.addUpdateAccountBalanceStmt != nil {
		if cerr := q.addUpdateAccountBalanceStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing addUpdateAccountBalanceStmt: %w", cerr)
		}
	}
	...
	if q.listsEntriesStmt != nil {
		if cerr := q.listsEntriesStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing listsEntriesStmt: %w", cerr)
		}
	}
	if q.listsTransfersStmt != nil {
		if cerr := q.listsTransfersStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing listsTransfersStmt: %w", cerr)
		}
	}
...
type Queries struct {
	db                          DBTX
	tx                          *sql.Tx
	addUpdateAccountBalanceStmt *sql.Stmt
	...
	listsEntriesStmt            *sql.Stmt
	listsTransfersStmt          *sql.Stmt
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		db:                          tx,
		tx:                          tx,
		addUpdateAccountBalanceStmt: q.addUpdateAccountBalanceStmt,
		...
		listsEntriesStmt:            q.listsEntriesStmt,
		listsTransfersStmt:          q.listsTransfersStmt,
